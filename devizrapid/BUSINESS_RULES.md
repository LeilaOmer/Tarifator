...
- **Modelele se DEPRECIAZA la furnizor.** `meta-llama/llama-4-scout-17b-16e-instruct` a fost oprit de Groq pe 17.06.2026 (free/developer), iar scanarea pozelor a murit fara semnal, pentru ca UI-ul raporta orice eroare necunoscuta drept "poza neclara". De aceea modelele sunt o LISTA incercata in ordine, configurabila din mediu (valori separate prin virgula): `GROQ_VISION_MODEL` (implicit `qwen/qwen3.6-27b,meta-llama/llama-4-maverick-17b-128e-instruct`) si `GROQ_TEXT_MODEL` (implicit `llama-3.3-70b-versatile`). Codul trece la urmatorul model DOAR pe eroarea "model inexistent"; orice alta eroare (limita de rata, cerere prea mare) se propaga, ca sa nu ardem cota pe toate modelele pentru aceeasi problema. La urmatoarea depreciere pui alt ID in variabila din Vercel, fara deploy de cod. Verifica periodic https://console.groq.com/docs/deprecations. Pe planul GRATUIT: ~30.000 tokeni/minut si ~500.000 tokeni/zi per model.
