...
- **Garduri deterministe peste orice citire AI** (`lib/pricing/scanGuards.ts`): (1) liniile de garantie/ambalaj SGR se filtreaza SI in cod (modelul uneori le scapa ca produse); (2) `reconcileUnitPrice` alege pretul care satisface `cantitate x pret ≈ valoare`, tinand cont de discount (valoarea randului poate fi deja neta de discount) si de separatorul romanesc de mii ("4.560" = 4560 bucati — factorul 1000 dintre declarat si derivat inseamna cantitate citita gresit, nu pret gresit).
