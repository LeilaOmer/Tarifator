...
- **Modelele se DEPRECIAZA la furnizor.** `meta-llama/llama-4-scout-17b-16e-instruct` a fost oprit de Groq pe 17.06.2026 (free/developer), iar scanarea pozelor a murit fara semnal, pentru ca UI-ul raporta orice eroare necunoscuta drept "poza neclara". De aceea modelele sunt configurabile din mediu: `GROQ_VISION_MODEL` (implicit `qwen/qwen3.6-27b` — multimodal, mod JSON) si `GROQ_TEXT_MODEL` (implicit `llama-3.3-70b-versatile`). La urmatoarea depreciere schimbi variabila in Vercel, fara deploy de cod. Verifica periodic https://console.groq.com/docs/deprecations. Pe planul GRATUIT: ~30.000 tokeni/minut si ~500.000 tokeni/zi per model.
