...
import { generateTikTokContent } from '@/lib/tiktok/generate'

// Agent TikTok — pasul 1 (nucleu). Genereaza intr-un apel: idee, hook, scenariu,
// descriere, hashtaguri si un prompt pentru generatoare video. Deocamdata DOAR
// intoarce rezultatul (fara salvare, fara UI) — acelea vin la pasii urmatori.
//
...
export async function POST(req: NextRequest) {
  ...
  try {
    const content = await generateTikTokContent({ topic })
